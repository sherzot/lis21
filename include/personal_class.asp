...
Class clsP_Info
	...
	Public Sub Initialize()
		...
		'TOP 060823 Lis林 氏名検索バグ対応 REP
		'If GetForm("CONF_Name_1", 1) <> "" And GetForm("CONF_Name_2", 1) <> "" Then IsData = True: Name = GetForm("CONF_Name_1", 1) & "　" & GetForm("CONF_Name_2", 1)
		If Trim(GetForm("CONF_Name_1", 1)) <> "" And Trim(GetForm("CONF_Name_2", 1)) <> "" Then IsData = True: Name = Trim(GetForm("CONF_Name_1", 1)) & "　" & Trim(GetForm("CONF_Name_2", 1))
		...
		If Trim(GetForm("CONF_Name_F_1", 1)) <> "" And Trim(GetForm("CONF_Name_F_2", 1)) <> "" Then IsData = True: Name_F = Trim(GetForm("CONF_Name_F_1", 1)) & "　" & Trim(GetForm("CONF_Name_F_2", 1))
		...
		If Trim(GetForm("CONF_Name_1", 1)) <> "" And Trim(GetForm("CONF_Name_2", 1)) <> "" Then IsData = True: SearchName = Trim(GetForm("CONF_Name_1", 1)) & Trim(GetForm("CONF_Name_2", 1))
		...
		If Trim(GetForm("CONF_Name_F_1", 1)) <> "" And Trim(GetForm("CONF_Name_F_2", 1)) <> "" Then IsData = True: SearchName_F = Trim(GetForm("CONF_Name_F_1", 1)) & Trim(GetForm("CONF_Name_F_2", 1))
		'BTM 060823 Lis林 氏名検索バグ対応 REP
		...
		If GetForm("CONF_Town_F", 1) <> "" Then IsData = True: Town_F = GetForm("CONF_Town_F", 1)
		If GetForm("CONF_Address", 1) <> "" Then IsData = True: Address = GetForm("CONF_Address", 1)
		If GetForm("CONF_Address_F", 1) <> "" Then IsData = True: Address_F = GetForm("CONF_Address_F", 1)
		...
		If GetForm("CONF_UrgencyPost_U", 1) <> "" Then IsData = True: UrgencyPost_U = GetForm("CONF_UrgencyPost_U", 1)
		If GetForm("CONF_UrgencyPost_L", 1) <> "" Then IsData = True: UrgencyPost_L = GetForm("CONF_UrgencyPost_L", 1)
		If GetForm("CONF_UrgencyAddress", 1) <> "" Then IsData = True: UrgencyAddress = GetForm("CONF_UrgencyAddress", 1)
		If GetForm("CONF_UrgencyAddress_F", 1) <> "" Then IsData = True: UrgencyAddress_F = GetForm("CONF_UrgencyAddress_F", 1)
		If GetForm("CONF_UrgencyTelephoneNumber", 1) <> "" Then IsData = True: UrgencyTelephoneNumber = GetForm("CONF_UrgencyTelephoneNumber", 1)
		If GetForm("CONF_URL", 1) <> "" Then IsData = True: URL = GetForm("CONF_URL", 1)
		If GetForm("CONF_InfoSource", 1) <> "" Then IsData = True: InfoSourceType = GetForm("CONF_InfoSource", 1)
		...
		If StaffCode <> "" And IsMainCode(StaffCode) = False Then
			Err = Err & "コードが発番されませんでした。<br>"
		End If

		'名前
		If Name = "" Or ChkLen(Name, 100) = False Then
			ErrStyle("CONF_Name_1") = "background-color:#ffff00;"
			ErrStyle("CONF_Name_2") = "background-color:#ffff00;"
			Err = Err & "名前が不正です。<br>"
		End If

		'名前カナ
		If Name_F = "" Or ChkLen(Name_F, 100) = False Then
			ErrStyle("CONF_Name_F_1") = "background-color:#ffff00;"
			ErrStyle("CONF_Name_F_2") = "background-color:#ffff00;"
			Err = Err & "名前カナが不正です。<br>"
		End If

		'誕生日
		If Birthday <> "" And IsDay(Birthday) = False Then
			ErrStyle("CONF_Birthday") = "background-color:#ffff00;"
			Err = Err & "誕生日の日付が不正です。<br>"
		End If

		'性別
		If Sex <> "" And IsRE(Sex, "^[12]$", True) = False Then
			ErrStyle("CONF_Sex") = "background-color:#ffff00;"
			Err = Err & "性別を入力してください。<br>"
		End If

		'
		If MarriageFlag <> "" And IsFlag(MarriageFlag) = False Then
			ErrStyle("CONF_MarriageFlag") = "background-color:#ffff00;"
			Err = Err & "扶養が不正です。<br>"
		End If

		'郵便番号
		If Post_U & Post_L <> "" And IsNumber(Post_U & Post_L, 7, False) = False Then
			ErrStyle("CONF_Post_U") = "background-color:#ffff00;"
			ErrStyle("CONF_Post_L") = "background-color:#ffff00;"
			Err = Err & "郵便番号が不正です。<br>"
		End If

		'都道府県コード
		If PrefectureCode <> "" And IsNumber(PrefectureCode, 3, False) = False Then
			ErrStyle("CONF_PrefectureCode") = "background-color:#ffff00;"
...
Class clsP_UserInfo
	...
	Public Sub Initialize()
		...
		If GetForm("CONF_NoticeMailFlag", 1) <> "" Then IsData = True: NoticeMailFlag = GetForm("CONF_NoticeMailFlag", 1)
...
Class clsP_SelfPR
	...
	Public Sub Initialize()
		...
		IsData = True
		If GetForm("CONF_StaffCode", 1) <> "" Then StaffCode = GetForm("CONF_StaffCode", 1)
		If GetForm("CONF_SelfPR", 1) <> "" Then SelfPR = GetForm("CONF_SelfPR", 1)
	End Sub
...
Class clsP_CareerHistory
	...
	Public Sub Initialize()
		...
		Do While True
			...
			If sEntryDay <> "" Then
				...
				ReDim Preserve Capital(MaxIndex) : Capital(MaxIndex) = GetForm("CONF_Capital" & idx, 1)
				ReDim Preserve NumberEmployees(MaxIndex) : NumberEmployees(MaxIndex) = GetForm("CONF_NumberEmployees" & idx, 1)
				ReDim Preserve Summary(MaxIndex) : Summary(MaxIndex) = GetForm("CONF_Summary" & idx, 1)
				ReDim Preserve EntryNote(MaxIndex):EntryNote(MaxIndex) = GetForm("CONF_EntryNote" & idx,1)
				ReDim Preserve RetireNote(MaxIndex):RetireNote(MaxIndex) = GetForm("CONF_RetireNote" & idx,1)
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		If IsData = False Then Exit Function
		For idx = 0 To MaxIndex
			GetRegSQL = GetRegSQL & "EXEC up_Reg_P_CareerHistory" & _
				" '" & vStaffCode & "'" & _
				",''" & _
				...
				",'" & RetireReason(idx) & "'" & _
				",'" & Capital(idx) & "'" & _
				",'" & NumberEmployees(idx) & "'" & _
				",'" & Summary(idx) & "'" & _
				",'" & EntryNote(idx) & "'" & _
				",'" & RetireNote(idx) & "'" & vbCrLf
		Next
	End Function
End Class
...
Class clsP_DevelopmentTool
	...
	Public Sub Initialize(vCategoryCode)
		...
		Do While True
			...
			If flag = True And GetForm("CONF_DevelopmentTool_" & vCategoryCode & idx, 1) <> "" Then
				MaxIndex = MaxIndex + 1
				ReDim Preserve CareerHistoryID(MaxIndex)
				ReDim Preserve Code(MaxIndex)
				If flag = True Then
					CareerHistoryID(MaxIndex) = iCareerHistoryID
					Code(MaxIndex) = Split(GetForm("CONF_DevelopmentTool_" & vCategoryCode & idx, 1), ",")
				End If
			End If
	...
	Public Sub Initialize()
		...
		Do While True
			...
			If GetForm("CONF_LicenseCode" & idx, 1) <> "" Then
				...
				ReDim Preserve LicenseNameDsp(MaxIndex) : LicenseNameDsp(MaxIndex) = GetForm("CONF_LicenseNameDsp" & idx, 1)
...
Class clsP_HopeWorkingPlace
	...
	Public Sub Initialize()
		...
		Do While True
			...
			If GetForm("CONF_HopePrefecture" & idx, 1) <> "" Then
                If CInt(GetForm("CONF_HopePrefecture" & idx, 1)) < 90  Then
				    IsData = True
				    MaxIndex = MaxIndex + 1
                ...
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "090" Then'全国
                    IsData = True
                    Zenkoku = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "091" Then'東北
                    IsData = True
                    Touhoku = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "092" Then'関東
                    IsData = True
                    Kanto = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "093" Then'甲信越
                    IsData = True
                    Koushinetu = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "094" Then'北陸
                    IsData = True
                    Hokuriku = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "095" Then'東海地方
                    IsData = True
                    Toukai = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "096" Then'関西
                    IsData = True
                    Kansai = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "097" Then'中国
                    IsData = True
                    Cyuugoku = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "098" Then'四国
                    IsData = True
                    Shikoku = true
                ElseIF GetForm("CONF_HopePrefecture" & idx, 1) = "099" Then'九州
                    IsData = True
                    Kyuusyuu = true
                End If
			End If
	...
	Public Function GetRegSQL(ByVal vStaffCode)
        ...
        If Zenkoku = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceZenkoku '"  & vStaffCode & "'" & vbCrLf
        End If

        If Touhoku = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceTouhoku '"  & vStaffCode & "'" & vbCrLf
        End If

        If Kanto = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceKanto '"  & vStaffCode & "'" & vbCrLf
        End If

        If Koushinetu = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceKoushinetu '"  & vStaffCode & "'" & vbCrLf
        End If

        If Hokuriku = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceHokuriku '"  & vStaffCode & "'" & vbCrLf
        End If

        If Toukai = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceToukai '"  & vStaffCode & "'" & vbCrLf
        End If

        If Kansai = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceKansai '"  & vStaffCode & "'" & vbCrLf
        End If

        If Cyuugoku = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceCyuugoku '"  & vStaffCode & "'" & vbCrLf
        End If

        If Shikoku = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceShikoku '"  & vStaffCode & "'" & vbCrLf
        End If

        If Kyuusyuu = true Then
	        GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeWorkingPlaceKyuusyuu '"  & vStaffCode & "'" & vbCrLf
        End If
		If IsData = False Then Exit Function
		For idx = 0 To MaxIndex
...
Class clsP_Note
	...
	'******************************************************************************
	Public Function GetRegSQL(ByVal vStaffCode)
		GetRegSQL = "EXEC sp_Del_P_Note '" & vStaffCode & "', '" & CategoryCode & "', '" & Code & "'" & vbCrLf
		If IsData = False Then Exit Function
...
Class clsP_ResumeOther
	...
	Public Sub Initialize()
		...
		Do While True
			...
			If GetForm("CONF_Subject" & idx, 1) & GetForm("CONF_WishMotive" & idx, 1) & GetForm("CONF_CommuteTime" & idx, 1) & GetForm("CONF_HopeColumn" & idx, 1) <> "" Then
				MaxIndex = MaxIndex + 1

				'ReDim Preserve PrintFlag(MaxIndex)
				'If CInt(GetForm("CONF_PrintFlag", 1)) = MaxIndex Then
				'	PrintFlag(MaxIndex) = "1"
				'Else
				'	PrintFlag(MaxIndex) = "0"
				'End If

				ReDim Preserve Subject(MaxIndex) : Subject(MaxIndex) = GetForm("CONF_Subject" & idx, 1)
				ReDim Preserve WishMotive(MaxIndex) : WishMotive(MaxIndex) = GetForm("CONF_WishMotive" & idx, 1)
				ReDim Preserve CommuteTime(MaxIndex) : CommuteTime(MaxIndex) = GetForm("CONF_CommuteTime" & idx, 1)
				ReDim Preserve HopeColumn(MaxIndex) : HopeColumn(MaxIndex) = GetForm("CONF_HopeColumn" & idx, 1)
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "EXEC sp_Del_P_ResumeOther '" & (vStaffCode) & "'"
		If IsData = False Then Exit Function
		For idx = 0 To MaxIndex
...
Class clsP_ResumeStudent
	...
	Public Sub Initialize()
		...
		If GetForm("CONF_StaffCode", 1) <> "" Then StaffCode = GetForm("CONF_StaffCode", 1)
		If GetForm("CONF_ResumeGood", 1) <> "" Then IsData = True: Good = GetForm("CONF_ResumeGood", 1)
		If GetForm("CONF_ResumeHealth", 1) <> "" Then IsData = True: Health = GetForm("CONF_ResumeHealth", 1)
		If GetForm("CONF_ResumeActivity", 1) <> "" Then IsData = True: Activity = GetForm("CONF_ResumeActivity", 1)
		If GetForm("CONF_ResumeSpecialty", 1) <> "" Then IsData = True: Specialty = GetForm("CONF_ResumeSpecialty", 1)
		If GetForm("CONF_ResumeLearn", 1) <> "" Then IsData = True: Learn = GetForm("CONF_ResumeLearn", 1)
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		If Good & Health & Activity & Specialty = "" Then
			GetRegSQL = "sp_Del_P_ResumeStudent '" & vStaffCode & "'"
			Exit Function
		End If
...
Class clsN_UserInfo
	...
	Public Sub Initialize()
		...
		If GetForm("CONF_StaffCode", 1) <> "" Then StaffCode = GetForm("CONF_StaffCode", 1)
		If GetForm("CONF_PenName", 1) <> "" Then PenName = GetForm("CONF_PenName", 1)
		If GetForm("CONF_Password", 1) <> "" Then Password = GetForm("CONF_Password", 1)
		If GetForm("CONF_GraduateYear", 1) <> "" Then GraduateYear = GetForm("CONF_GraduateYear", 1)
		If GetForm("CONF_OperateClassComCode", 1) <> "" Then OperateClassComCode = GetForm("CONF_OperateClassComCode", 1)
		If GetForm("CONF_OperateClassWebCode", 1) <> "" Then OperateClassWebCode = GetForm("CONF_OperateClassWebCode", 1)
		If GetForm("CONF_OperateClassRemark", 1) <> "" Then OperateClassRemark = GetForm("CONF_OperateClassRemark", 1)
		If GetForm("CONF_BranchCode", 1) <> "" Then BranchCode = GetForm("CONF_BranchCode", 1)
		If GetForm("CONF_EmployeeCode", 1) <> "" Then EmployeeCode = GetForm("CONF_EmployeeCode", 1)
		If GetForm("CONF_MailMagazineFlag", 1) <> "" Then MailMagazineFlag = GetForm("CONF_MailMagazineFlag", 1)
		If GetForm("CONF_NewJohoMailFlag", 1) <> "" Then NewJohoMailFlag = GetForm("CONF_NewJohoMailFlag", 1)
		If GetForm("CONF_SuspensionFlag", 1) <> "" Then SuspensionFlag = GetForm("CONF_SuspensionFlag", 1)
		If GetForm("CONF_ErasureFlag", 1) <> "" Then ErasureFlag = GetForm("CONF_ErasureFlag", 1)
		If GetForm("CONF_NaviUseFlag", 1) <> "" Then NaviUseFlag = GetForm("CONF_NaviUseFlag", 1)
		If GetForm("CONF_HomeContactFlag", 1) <> "" Then HomeContactFlag = GetForm("CONF_HomeContactFlag", 1)
		If GetForm("CONF_PortableContactFlag", 1) <> "" Then PortableContactFlag = GetForm("CONF_PortableContactFlag", 1)
		If GetForm("CONF_MailContactFlag", 1) <> "" Then MailContactFlag = GetForm("CONF_MailContactFlag", 1)
		If GetForm("CONF_PersonDangerFlag", 1) <> "" Then PersonDangerFlag = GetForm("CONF_PersonDangerFlag", 1)
		If GetForm("CONF_HopeCommuteTime", 1) <> "" Then HopeCommuteTime = GetForm("CONF_HopeCommuteTime", 1)
		If GetForm("CONF_NoticeMailFlag", 1) <> "" Then NoticeMailFlag = GetForm("CONF_NoticeMailFlag", 1)
		If GetForm("CONF_LisReserveDay", 1) <> "" Then LisReserveDay = GetForm("CONF_LisReserveDay", 1)
		If GetForm("CONF_LisRegistDay", 1) <> "" Then LisRegistDay = GetForm("CONF_LisRegistDay", 1)
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "up_Reg_N_UserInfo '" & vStaffCode & "'" & _
			",'" & PenName & "'" & _
			",'" & Password & "'" & _
			",'" & GraduateYear & "'" & _
			",'" & OperateClassComCode & "'" & _
...
Class clsN_Image
	...
	Public Sub Initialize(ByVal vCategoryCode)
		...
		If GetForm("conf_" & CategoryCode, 1) <> "" Then sTemp = Replace(GetForm("conf_" & CategoryCode, 1), " ", "")
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "EXEC up_Del_N_Image '" & vStaffCode & "', '" & CategoryCode & "'" & vbCrLf
		If IsData = False Then Exit Function
		For idx = 0 To MaxIndex
			GetRegSQL = GetRegSQL & "EXEC up_Reg_N_Image '" & vStaffCode & "'" & _
				",'" & CategoryCode & "'" & _
				",'" & Code(idx) & "'" & vbCrLf
		Next
	End Function
...
Class clsN_School
	...
	Public Sub Initialize()
		...
		If GetForm("conf_staffcode", 1) <> "" Then StaffCode = GetForm("conf_staffcode", 1)
		If GetForm("conf_schoolcode", 1) <> "" Then SchoolCode = GetForm("conf_schoolcode", 1)
		If GetForm("conf_otherschoolname", 1) <> "" Then OtherSchoolName = GetForm("conf_otherschoolname", 1)
		If GetForm("conf_department", 1) <> "" Then Department = GetForm("conf_department", 1)
		If GetForm("conf_studyroom", 1) <> "" Then Studyroom = GetForm("conf_studyroom", 1)
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "up_Reg_N_School '" & vStaffCode & "'" & _
			",'" & SchoolCode & "'" & _
			",'" & OtherSchoolName & "'" & _
			",'" & Department & "'" & _
			",'" & Studyroom & "'" & vbCrLf
...
'******************************************************************************
'名　称：clsP_HopeContactableTime
'概　要：formで飛んできた希望連絡時間用テーブルのデータを持つためのクラス
'備　考：
'作成者：Lis Kimura
'作成日：2015/07/21
'******************************************************************************
Class clsP_HopeContactableTime
	Public StaffCode
	Public ContactableTimeCode()
	Public IsData
	Public MaxIndex
	Public Err
	...
	'******************************************************************************
	'名　称：Initialize
	'概　要：clsP_HopeContactableTime クラスの初期化関数
	'備　考：
	'作成者：Lis Kimura
	'作成日：2015/07/21
	'******************************************************************************
	Public Sub Initialize()
		...
		Do While True
			If ExistsForm("CONF_HopeContactableTime" & idx) = False Then Exit Do

			If GetForm("CONF_HopeContactableTime" & idx, 1) <> "" Then
				IsData = True
				MaxIndex = MaxIndex + 1

				ReDim Preserve ContactableTimeCode(MaxIndex) : ContactableTimeCode(MaxIndex) = GetForm("CONF_HopeContactableTime" & idx, 1)

				If ContactableTimeCode(MaxIndex) <> "" And IsNumber(ContactableTimeCode(MaxIndex), 3, False) = False Then Err = Err & "ContactableTimeCode(" & MaxIndex & ")" & vbCrLf
			End If
			idx = idx + 1
		Loop
	End Sub
	...
	'******************************************************************************
	'名　称：GetRegSQL
	'概　要：sp_Reg_P_HopeContactableTime 実行SQL取得
	'備　考：
	'作成者：Lis Kokubo
	'作成日：2006/03/24
	'更　新：
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "EXEC sp_Del_P_HopeContactableTime '" & (vStaffCode) & "'" & vbCrLf
		If IsData = False Then Exit Function
		For idx = 0 To MaxIndex
			GetRegSQL = GetRegSQL & "EXEC sp_Reg_P_HopeContactableTime" & _
				" '" & vStaffCode & "'" & _
				",''" & _
				",'" & ContactableTimeCode(idx) & "'"
		Next
	End Function
End Class
...
Class clsP_SideJob
	...
	Public Sub Initialize()
        ...
        If GetForm("CONF_SideJob", 1) <> "" Then 
            IsData = True
            Doit = GetForm("CONF_SideJob", 1)
        elseif GetForm("CONF_SideJob_s", 1) <> "" Then 
            IsData = True
            Doit = GetForm("CONF_SideJob_s", 1)
        end if

        '希望しない1 希望する2
        If Doit = "2" then

            'チェックしたら1、しないと空なので0をセット
            If GetForm("CONF_AtHome", 1) <> "" Then 
                AtHome = GetForm("CONF_AtHome", 1)
            Else
                AtHome = "0"
            End If

            If GetForm("CONF_Attendance", 1) <> "" Then 
                Attendance = GetForm("CONF_Attendance", 1)
            Else
                Attendance = "0"
            End If

        
            If GetForm("CONF_WorkDays", 1) <> "" Then IsData = True: WorkDays = GetForm("CONF_WorkDays", 1)
            If GetForm("CONF_Income", 1) <> "" Then IsData = True: Income = GetForm("CONF_Income", 1)
            If GetForm("CONF_Contents", 1) <> "" Then IsData = True: Contents = GetForm("CONF_Contents", 1)
        Else
            '副業希望しない場合はチェックなし、テキストクリアする。
            AtHome = "0"
            Attendance = "0"
            WorkDays = ""
            Income = ""
	...
	Public Function GetRegSQL(ByVal vStaffCode)
		...
		GetRegSQL = "sp_Reg_P_SideJob '" & vStaffCode & "'" & _
			",'" & Doit & "'" & _
			",'" & AtHome & "'" & _
			",'" & Attendance & "'" & _
            ",'" & WorkDays & "'" & _
