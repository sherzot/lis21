...
'　　　：
'　　　：■■■　GoogleJobs用構造化データセット　■■■
'　　　：SetGoogleJobs			：GoogleJobs用構造化データセット
'**********************************************************************************************************************
...
<p class="m0">※詳しい会社情報は下のボタンやお電話などで直接お問合せください。</p>
...
<% If sWorkingType <> "" Then
 ...
 %>
   ...
   <% If sYearlyIncome <> "" Then %>
    ...
    <% If sYearlyIncome & sMonthlyIncome & sDailyIncome & sHourlyIncome <> "" AND dbOrderCode <> "J0109133" AND (Instr(sWorkingType,"FC") = 0 AND Instr(sWorkingType,"自営") = 0 ) Then %>
...
Function DspHowToEntry2(ByRef rDB, ByRef rRS, ByVal vUserType, ByVal vUserID)
	...
	<table class="jCodeOnly">
    	<thead>
        	<td colspan="2">※求人に関するお問い合わせの際には、必ず下記の「情報コード」をお知らせください。「しごとナビを見た」とおっしゃって頂けますとスムースです。</td>
<!-- お祝い金文言を駆除
...
Function AccessHistoryOrder(ByRef rDB, ByRef rRS, ByVal vUserType, ByVal vUserID, ByVal vOrderCode)
    ...
    '一時的にログを残す アップ時は↑に切り替える！！！！！
    If (IsRE(G_IPADDRESS, "^192.168.", True) = False And vUserID <> "S0018066") or vUserID = "S0786205" Then
		If vUserType = "staff" Then
			sSQL = "up_Reg_LOG_AccessHistoryOrder '" & vOrderCode & "', '" & vUserID & "'"
			flgQE = QUERYEXE(dbconn, oRS, sSQL, sError)
			Call RSClose(oRS)

            '閲覧されたことを担当に通知(メール)する
            Call SendMail_AccessNotification(vUserID,vOrderCode,rRS)
...
'*******************************************************************************
'概　要：求職者の求人閲覧時、営業担当者にメール(通知)する
'引　数：sUserID    求職者コード
'      ：sOrderCode 求人票コード
'      ：rRS        p_SearchOrder or 求人票詳細検索SQL で生成されたレコードセットオブジェクト
'                   呼び出し元で使用しているものをそのまま使用する。
'作成日：2017/11/20 
'*******************************************************************************
Function SendMail_AccessNotification(ByVal sUserID,ByVal sOrderCode,ByVal rRS)
    ...
    Dim sError
    Dim sTo					'送信先メールアドレス
    Dim sFrom				'送信元メールアドレス
    Dim sSubject			'生成されたメール件名
    Dim sBody				'生成されたメール内容
    Dim sResult				'Basp21.SendMailの戻り値を保持

    '求人情報からリスの営業担当者メールアドレスを取得
    sSQL = ""
    sSQL = sSQL & "Select BE.MailAddress " 
    sSQL = sSQL & "From B_EmployeeMasterLis BE,C_Info CI "
    sSQL = sSQL & "Where BE.EmployeeCode = CI.EmployeeCode "
    sSQL = sSQL & "And CI.OrderCode = '" & sOrderCode & "';"
	flgQE = QUERYEXE(dbconn, oRS, sSQL, sError)

	'あて先に営業担当メールアドレスを取得
    If GetRSState(oRS) = True Then
        sTo = ChkStr( oRS.Collect("MailAddress") )
    End If
    Call RSClose(oRS)

    sFrom = Cnt_LisMailAddress

    'メール内容定義
    sSubject = ""
    sBody = ""
    sSubject = "【通知】担当求人が閲覧されました。"
    sBody = sBody & "担当の求人が求職者に閲覧されました。" & vbCrLf & vbCrLf

    sBody = sBody & "企業名：" & rRS.Collect("CompanyName") & vbCrLf
    sBody = sBody & "閲覧された受注表リンク：" & HTTP_BI_CURRENTURL & "collection/collection_detail.asp?OrderCode=" & sOrderCode & vbCrLf
    sBody = sBody & "閲覧した求職者リンク：" & HTTP_BI_CURRENTURL & "staff/staff_detail.asp?StaffCode=" & sUserID & vbCrLf & vbCrLf
...
'*******************************************************************************
'概　要：GoogleJobs用構造化データセット
'作成日：2019/07/09 
'*******************************************************************************
Function SetGoogleJobs(ByRef rDB, ByRef rRS, ByVal vUserType, ByVal vUserID, ByRef rTitle)
    ...
    Dim iMaxRow
	Dim sWorkingPlace
	Dim sNearbyStationBlock


	Response.write"	<!-- GoogleForJobs tags -->"& vbCrLf
	Response.write"	<script type=""application/ld+json""> {"& vbCrLf
	Response.write"	""@context"" : ""http://schema.org/"","& vbCrLf
	Response.write"	""@type"" : ""JobPosting"","& vbCrLf
	Response.write"	""title"" : """ & rRS.Collect("JobTypeDetail") & ""","& vbCrLf
	Response.write"	""description"" : """ & rRS.Collect("PRTitle1") & ""& vbCrLf & rRS.Collect("PRContents1") & "<br />"& vbCrLf
	Response.write rRS.Collect("PRTitle2") & ""& vbCrLf & rRS.Collect("PRContents2") & "<br />"& vbCrLf
	Response.write rRS.Collect("PRTitle3") & ""& vbCrLf & rRS.Collect("PRContents3") & "<br />"& vbCrLf
	Response.write "担当業務" & ""& vbCrLf & rRS.Collect("BusinessDetail") & "<br />"& vbCrLf
	Response.write""","& vbCrLf

	Response.write"	""datePosted"" : """ & GetDateStr(oRS.Collect("RegistDay"), "-") & " 00:00:00"","& vbCrLf
	Response.write"	""validThrough"" : """ & GetDateStr(oRS.Collect("PublicLimitDay"), "-") & " 23:59:59"","& vbCrLf

	Response.write"	""hiringOrganization"": {"& vbCrLf
	Response.write"	  ""@type"" : ""Organization"","& vbCrLf
	Response.write"	  ""name"" : ""社名非公開"","& vbCrLf
	Response.write"	  ""sameAs"" : """ & " " & ""","& vbCrLf
	Response.write"	  ""logo"" : ""https://www.shigotonavi.co.jp/img/navio.gif""" & ""& vbCrLf
	Response.write"	},"& vbCrLf

	Response.write"	""jobLocation"": {"& vbCrLf
	Response.write"	  ""@type"" : ""Place"","& vbCrLf
	Response.write"	  ""address"": {"& vbCrLf
	Response.write"	    ""@type"" : ""PostalAddress"","& vbCrLf
	Response.write"	    ""streetAddress"" : """ & " " & ""","& vbCrLf
	'Response.write"	    ""addressLocality"" : """ & " " & ""","& vbCrLf
	'Response.write"	    ""addressRegion"" : """ & " " & ""","& vbCrLf
	
	iMaxRow = 0
	sWorkingPlace = ""
	sNearbyStationBlock = ""
	sSQL = "EXEC up_LstC_WorkingPlace '" & rRS.Collect("OrderCode") & "';"
	flgQE = QUERYEXE(rDB, oRS2, sSQL, sError)
	If GetRSState(oRS) = True Then
		'Response.write"	    ""streetAddress"" : """ & oRS2.Collect("WorkingPlaceAddressAll") & ""","& vbCrLf
		Response.write"	    ""addressLocality"" : """ & oRS2.Collect("WorkingPlaceCity") & ""","& vbCrLf
		Response.write"	    ""addressRegion"" : """ & oRS2.Collect("WorkingPlacePrefectureName") & ""","& vbCrLf
    End If

	Response.write"	    ""postalCode"" : """ & " " & ""","& vbCrLf
	Response.write"	    ""addressCountry"" : """ & "JP" & """"& vbCrLf
	Response.write"	  }"& vbCrLf
	Response.write"	},"& vbCrLf

	Response.write"	""identifier"": {"& vbCrLf
	Response.write"	  ""@type"" : ""PropertyValue"","& vbCrLf
	Response.write"	  ""name"" : ""社名非公開"","& vbCrLf
	Response.write"	  ""value"" : """ & rRS.Collect("OrderCode") & """"& vbCrLf
	Response.write"	}"& vbCrLf
